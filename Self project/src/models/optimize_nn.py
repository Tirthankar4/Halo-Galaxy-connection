...
def create_objective(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for train_idx, val_idx in kf.split(X_train):
            ...
            val_metric = compute_single_metric(y_val, val_pred, metric)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Galaxy property targets to optimize (default: {TARGET_COLUMNS}). Available: {ALL_GALAXY_PROPERTIES}",
    )
    parser.add_argument(
        "--n-trials",
        type=int,
...
def main() -> None:
    ...
    logging.info("Starting hyperparameter optimization for targets: %s", targets)
    logging.info("Using features: %s", features)
