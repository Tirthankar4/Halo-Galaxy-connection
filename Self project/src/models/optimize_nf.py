...
def compute_metric_per_property(
    ...
    for prop_idx in range(y_true.shape[1]):
        prop_metric = compute_single_metric(y_true[:, prop_idx], y_pred[:, prop_idx], metric)
...
def optimize_and_save_best_model(
    ...
    Args:
        ...
        targets: List of target column names to predict
    ...
    logging.info("Using targets: %s", targets)
    ...
    for idx, prop in enumerate(targets):
        metrics[prop] = compute_regression_metrics(y_true[:, idx], predictions["mean"][:, idx])
